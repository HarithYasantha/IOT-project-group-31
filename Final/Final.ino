...
#define MQ_ANALOG_PIN 34       // A0 on MQ sensor -> GPIO34
#define MQ_THRESHOLD 500       // Adjust based on testing
...
void setupAlcoholSensor() {
  ...
  analogReadResolution(10); // For ESP32, default is 12-bit
  Serial.println("✅ MQ alcohol sensor initialized");
}
...
void loop() {
  // --- Alcohol Detection ---
  int alcoholValue = analogRead(MQ_ANALOG_PIN);
  Serial.print("Alcohol sensor value: ");
  Serial.println(alcoholValue);

  if (alcoholValue > MQ_THRESHOLD) {
    Serial.println("🚫 Alcohol detected! Blocking crash detection.");
  ...
  // --- Read IR Sensor ---
  helmetOn = digitalRead(IR_PIN) == LOW;
  if (!helmetOn) {
